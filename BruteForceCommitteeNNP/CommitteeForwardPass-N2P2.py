...
print("Hey Did you change the mean energy")
...
#Change the variables everytime you do predictions.
#Copy scaling.data from NN1 scalingdata folder
#Load function.cleaned.data from input_files
#Load weights from each NN folder to make predictions

el = [1, 6]

NOFNODES = 25

NOFATOMS = {'H': 64, 'C': 80} #as a dict
#NOFATOMS_TEST = {'H': 4, 'O': 2} #as a dict

#conv_energy =  35.207980028719568       #check this value before implementing
mean_energy = -582.83487509638849 

true_energy = np.loadtxt('../input_files/energy.data').reshape(-1, 1)
...
def symmetryfunction(FUNCTION, SCALING):
    ...
    el = np.asarray(sorted(list({x[0] for x in data})), dtype = int)    #expensive loop can somehow be mearged with below loop
    ...
    scaledsf = {id2el[i]: ((sf[id2el[i]] - scalingmat[id2el[i]][2]) / scalingmat[id2el[i]][3]) for i in el} #scaled symmetry functions
    
    end = time.time()

    print(f'Time to load symmetry functions  {(end - start)/60} min')
    return scaledsf, el, NOFSYM

#To test the symmetryfunction function uncomment the following lines:

scaledsf, el, NOFSYM = symmetryfunction(FUNCTION = '../input_files/function.cleaned.data', SCALING = '../scalingdata/scaling.data')
...
def forwardpass(scaledsf, weightmat, el):
    ...
    for i in el:
        ...
        for j in range(scaledsf[id2el[i]].shape[0]):
            ...
            y1 = activation(y1, 'softplus')

            y2 = (w1.T @ y1) + b1
            y2 = activation(y2, 'softplus')
    ...
    print(f'Time for the forward pass {(end - start)/ 60} min')
    return energycontri


#energycontri = forwardpass(scaledsf = scaledsf, weightmat = weights, el = el)
...
def feature_extractor():
    ...
    weights = weightmat(wlist = sorted(glob.glob('weight*')), el = el, NOFSYM = NOFSYM)
...
NNs = sorted(glob.glob('/u/shubsharma/HDNNP/Naphthalene-Multi/predictions/testing-newstyle/mulpred/NN*'))

print('\n************************ Starting forward pass for the Committee ************************************\n')
for i in NNs:
    os.chdir(i)

    energycontri = feature_extractor()
    
    conv_energy = np.loadtxt('conv_energy.txt')
    print(f'Conv_energy {conv_energy}\n')
    eh = np.sum((energycontri['H']/conv_energy) + mean_energy, axis = 1)
    eo = np.sum((energycontri['C']/conv_energy) + mean_energy, axis = 1)

    totalenergy = eh + eo
    
    np.savetxt(f'forward_pass.txt', np.concatenate((true_energy, totalenergy), axis = 1), delimiter='\t', newline='\n', comments='# ', header=f'true_energy\tn2p2_energy', fmt = '%.8f')

endpred = time.time()
print(f'\n Total time for the predictions for all NNs {(endpred - startpred)/60} min')

#
#datasets = sorted(glob.glob(os.path.join('/Users/shubhamdongriyal/my-drive/SabIA/HDNNP/H2O-Bing/MulRegression/trainset/seed-1234567/trainset-50%/data-*')), key= lambda x: int(x.split('-')[-1]))
#
#def committee():
#    committee_model = {}
...
##TESTING STAGE
#
#feature_test = feature_extractor(FUNCTION = os.path.join('/Users/shubhamdongriyal/my-drive/SabIA/HDNNP/H2O-Bing/MulRegression/testset/test-dimer/function.cleaned.data'), SCALING = os.path.join('/Users/shubhamdongriyal/my-drive/SabIA/HDNNP/H2O-Bing/MulRegression/testset/test-dimer/scaling.data'))
#energy_test = np.loadtxt(os.path.join('/Users/shubhamdongriyal/my-drive/SabIA/HDNNP/H2O-Bing/MulRegression/testset/test-dimer/energy.data'), usecols=0, dtype=float).reshape(-1, 1)
...
#committee_std = np.std(energy_mat, axis = 1).reshape(-1, 1)
#
#n2p2_energy = np.loadtxt('/Users/shubhamdongriyal/my-drive/SabIA/HDNNP/H2O-Bing/MulRegression/testset/test-dimer/n2p2_energy_frames.out', usecols=0, dtype=float).reshape(-1, 1)
#np.savetxt('/Users/shubhamdongriyal/Desktop/predicted_energy_trained-1234567.txt', np.concatenate((energy_test, np.concatenate((list(total_energy.values())), axis = 1)), axis =1), delimiter='\t', newline='\n', comments='# ', header=f'true_energy\tcommittee_energy_predictions', fmt = '%.8f')
#np.savetxt('/Users/shubhamdongriyal/Desktop/committee_mean_std_trained-1234567.txt', np.concatenate((energy_test, n2p2_energy, committee_mean, committee_std), axis = 1), delimiter='\t', newline='\n', comments='# ', header=f'true_energy\tn2p2_energy\tcommittee_mean\tcommittee_std', fmt = '%.8f')
